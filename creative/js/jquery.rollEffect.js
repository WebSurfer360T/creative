( function( $, window, undefined ) {
	...
	$.rollEffect = function( options, element ) {
		this.$el = $( element );
		this._init( options );
 	};
 	$.rollEffect.defaults = {
 	...
 	$.rollEffect.prototype	= {
 		_init : function( options ) {
 			this.options = $.extend( true, {}, $.rollEffect.defaults, options );
		...
		_initEvents : function() {
 			var self = this;
 			this.$items.on( 'click.rollEffect', function( event ) {
				self._openItem( $( this ) );
			} );
 		},
 	...
 	$.fn.rollEffect			= function( options ) {
 		var instance = $.data( this, 'rollEffect' );
 		if ( typeof options === 'string' ) {
 			var args = Array.prototype.slice.call( arguments, 1 );
 			this.each(function() {
 				if ( !instance ) {
 					logError( "cannot call methods on rollEffect prior to initialization; " +
					"attempted to call method '" + options + "'" );
					return;
 				}
 				if ( !$.isFunction( instance[options] ) || options.charAt(0) === "_" ) {
 					logError( "no such method '" + options + "' for rollEffect instance" );
					return;
 				}
 			...
 			this.each(function() {
 				if ( instance ) {
 					...
 					instance = $.data( this, 'rollEffect', new $.rollEffect( options, this ) );
 				}
 			});
 		}
 		return instance;
